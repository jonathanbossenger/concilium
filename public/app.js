...
class GitHubCard {
  ...
  async runIssueAction(item, btn, action = 'assign_copilot') {
    ...
    if (!confirm(action === 'close'
      ? `Close issue #${item.number}?`
      : `Assign issue #${item.number} to Copilot?`)) return;
    btn.disabled = true;
    this.setStatus(`${actionProgress} #${item.number}…`, 'running');
    try {
      ...
      const data = await r.json().catch(() => ({}));
      if (!r.ok) {
        this.setStatus(data.error || `failed to ${actionFailed} #${item.number}`, 'err');
        return;
      }
      const successFallback = action === 'close'
        ? `issue #${item.number} closed`
        : `issue #${item.number} assigned`;
      this.setStatus(data.message || successFallback, 'ok');
      await this.load(this.currentUrl);
    } catch (err) {
      console.error('[concilium] issue action failed:', err);
      this.setStatus(`failed to ${actionFailed} #${item.number}`, 'err');
    } finally {
      btn.disabled = false;
    }
  }
